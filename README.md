...
- **`src/components/GlobeView.tsx`** — wraps `react-globe.gl`, progressively reveals visited markers during playback, and pulses a ring on the current / selected check-in. The camera is never moved programmatically — you always control rotation and zoom. All markers are one GPU point cloud (`THREE.Points` via globe.gl's custom layer): a single draw call for the whole history, drawn as soft translucent discs at a **constant on-screen size** (`sizeAttenuation` off, scaled by the "Marker size" slider). So markers never balloon when you zoom into a dense area — the cluster just spreads into individual dots. Hover/click hit-testing projects every near-side marker to the screen and takes the one nearest the pointer (a point-cloud raycast returns the marker nearest the *camera*, which mis-fires when zoomed in; globe.gl's per-object layer can't scale to thousands of markers anyway).
